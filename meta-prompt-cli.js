...
const mcpTools = {
  available: [
    'firecrawl_search', 'brave_web_search', 'logo_search', 
    'byterover-store-knowledge', 'sequentialthinking'
  ],
...
class MetaPromptGenerator {
  ...
  async buildPrompt(input) {
    ...
    const prompt = {
      ...
      output_format: this.buildOutputFormat(preferences.verbosity, constraints.format),
      examples: preferences.examples ? this.buildExamples(domain) : null,
      quality_criteria: this.buildQualityCriteria(),
      tools_integration: availableTools.length > 0 ? {
  ...
  buildInstructions(domain, userProfile, preferences) {
    ...
    return [
      ...
      preferences.tone ? `7. Maintain ${preferences.tone} tone throughout` : null
    ].filter(Boolean);
  }
  ...
  buildOutputFormat(verbosity = 'detailed', format = 'markdown') {
    ...
    return {
      ...
      structure: {
        required_sections: ["context", "instructions", "constraints", "quality_criteria"],
  ...
  async refinePrompt(prompt, issues) {
    ...
    if (issues.includes("Context section needs more detail")) {
      prompt.context += " This prompt is designed to provide comprehensive guidance while maintaining safety and quality standards.";
    }
...
async function main() {
  ...
  try {
    ...
    const expertise = await question('Your expertise level (beginner/intermediate/expert): ');
    const tone = await question('Preferred tone (professional/friendly/empathetic) [professional]: ') || 'professional';
    const verbosity = await question('Detail level (concise/detailed/comprehensive) [detailed]: ') || 'detailed';
    ...
    console.log(`Tools: ${generatedPrompt.tools_integration?.available_mcp_tools.length || 0} MCP servers available`);
...
export { MetaPromptGenerator, mcpTools, defaultConfig };
